...
\citep{peeta2010} constructs a model to evaluate the most efficient allocation of
...
\citep{ibrahim2011} provides an alternative heuristic approach for determining
...
\citep{omer2013} proposes a methodology for assessing the resiliency of physical
...
\citep{jaller2015} seeks to identify critical infrastructure based on increased
...
quantified in numerous ways. \citet{dong2006} provide a helpful framework for
...
destination is within an \emph{isochrone}, or certain distance. This measure is often
...
and \citep{xiangdong2007}; namely, the measure is based in actual choice theory, and
...
\citet{guers2004} provide a review of accessibility measures such as
...
their size (as represented by \(A_j\) in Equation \eqref{eq:1}). The friction
factor in Equation \eqref{eq:2} asks an implicit question with its distance or
...
through the Utah Household Travel Survey (UHTS) conducted in 2017 \citep{uhts2017}.
...
\tightlist
\item
  \emph{Highway Network}: The USTM model network, which was obtained from UDOT and
  forms the basis of the model.
\item
  \emph{Productions}: Zonal trip productions from the USTM model, classified as
  household productions.
\item
  \emph{Attractions}: Zonal trip attractions from the USTM model, classified as
  socioeconomic data.
\end{enumerate}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Skim Network - In this step, the model determines the shortest path by
...
\begin{figure}
\includegraphics[width=0.75\linewidth]{images/OGframework} \caption{Model framework.}\label{fig:framework}
\end{figure}

\hypertarget{highway-and-transit-skims}{%
\subsection{Highway and Transit Skims}\label{highway-and-transit-skims}}
...
\hypertarget{trip-attractions-and-socioeconomic-data}{%
\subsection{Trip Attractions and Socioeconomic Data}\label{trip-attractions-and-socioeconomic-data}}
...
of employment by job category \citep{uhts2017}. This information is useful when
...
\hypertarget{mode-choice-model}{%
\subsection{Mode Choice Model}\label{mode-choice-model}}
...
the utility equations \citep{koppelman2006}.

The utility equations from the Roanoke Valley Transportation Organization model
\citep{rvtpo} were extracted and then pared down to match the available data from
...
\hypertarget{destination-choice-model}{%
\subsection{Destination Choice Model}\label{destination-choice-model}}
...
The DC equation, seen in Equation \eqref{eq:7}, is made up of the MCLS calculated
...
of \protect\hyperlink{methodology}{methodology}. Feeding the MCLS value into the DC module is
...
\hypertarget{maybe-include-table-with-beginning-and-ending-calibration-values}{%
\subsection{MAYBE INCLUDE TABLE WITH BEGINNING AND ENDING CALIBRATION VALUES?}\label{maybe-include-table-with-beginning-and-ending-calibration-values}}
