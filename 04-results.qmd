...
# Results {#results}
In this, we summarize the results used to identify vulnerable links on Utah’s 
highway network. We also apply the model to scenarios where critical highway 
links are removed from the model network. This includes first a detailed 
analysis of a single scenario, where I-80 between Salt Lake and Tooele Counties 
is severed. We compare the model output to an alternative method that measures 
only the change in travel time and does not allow for mode or destination 
choice. The model was then applied to 41 individual link closure scenarios 
...
\@ref(tab:tooeletable). This shows that the resiliency model is effectively
capturing trips in the correct areas based on the percent comparison capture
rate in Table \@ref(tab:tooeletable). When we look at the travel time method of
...
```{r tooeletable, echo = FALSE, fig.cap = "Tooele County comparison of logsum and travel time methods."}
...
kbl(
  caption = "Detailed Analysis of Tooele Scenario",
  col.names = c("Purpose", 
  ...
  pack_rows( index = c("Passenger" = 3, "External / Freight" = 5), hline_after = F )
```

```{r tooelemap, fig.cap="Tooele county HBW utility loss.", cache = FALSE}
...
For external trips, freight trips, and recreation trips, these were all extracted
...
Figure \@ref(fig:tooelemap) shows....
...
locations are shown in Figure \@ref(fig:links-map), and were identified in a report
...
if(!knitr::is_latex_output()){
  ...
  ggplot(mapdata) +
    ...
    coord_sf(crs = st_crs(4326),  expand = FALSE) + 
    geom_sf(data = mapdata, inherit.aes = FALSE, size = 5, stroke = 1,
            aes(fill = value/1e8, shape = factor(sign(value), labels = c("Benefits", "Costs"))), 
                alpha = 0.5)  + 
    scale_shape_manual("Direction", values = c(21, 22), ) +
    theme(axis.line = element_line(color = NA))  + 
    scale_fill_viridis("Cost of closure ($M)", option = "magma", direction = -1) + 
     theme(axis.line = element_line(color = NA),
          axis.title.x=element_blank(),
          axis.text.x=element_blank(),
          axis.ticks.x=element_blank(),
...
This section will present the results of the 41 scenarios analyzed. The logsum method results 
are as follows in Table \@ref(tab:logsumrank). The results are ranked from the road with the 
largest cost to the road with the smallest cost. We can
