...
different. In the logsum-based method, "Inside Toole" are costs
...
This exercise reveals why such a practice should be discouraged, and
also highlights the importance of using consistent functions for
impedance calculation at all stages of the model.

```{r links-map, fig.cap="Total cost of link closure.", fig.show='hold'}
#| label: fig-linksmap
#| fig-cap: Total cost of link closure for each scenario.
tar_load(mapdata)
...
ggplot(mapdata) +
  ...
  theme(axis.line = element_line(color = NA))  + 
  viridis::scale_fill_viridis("Cost of closure ($M)", option = "magma", direction = -1) + 
   theme(axis.line = element_line(color = NA),
        axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),
